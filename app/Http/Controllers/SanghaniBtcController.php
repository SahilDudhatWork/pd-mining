...
use App\Models\BtcUserRefers;
use Illuminate\Support\Facades\Auth;
use App\Mail\OtpMail;
use Carbon\Carbon;


class SanghaniBtcController extends Controller
{
    public function registerUser(UserRegistrationRequest $request)
    {
        try {
            ...
            if(isset($request->refer_code) && !empty($request->refer_code)){
                $refer_user = SanghaniBtcUser::where('refer_code',$request->refer_code)->first();
                if($refer_user == null){
            ...
            $user = SanghaniBtcUser::create($validatedData);
            if($refer_user && isset($refer_user)){
                BtcUserRefers::create(['from_user_id' => $refer_user->id,'to_user_id' => $user->id]);
            }
    ...
    public function registerWithGoogle(GoogleRegistrationRequest $request)
    {
        try {
            $validatedData = $request->validated();
            $googleUser = SanghaniBtcUser::where('email',$validatedData['email'])->first();
            ...
            $getUser = SanghaniBtcUser::find($user->id);
   ...
   public function loginUser(UserLoginRequest $request)
    {
        try {
            $googleUser = SanghaniBtcUser::where('email',$request->email)->first();
            
            if((isset($googleUser) && !empty($googleUser)) && $googleUser->social_type == 'google'){
                return $this->responseError('google login', 401);
            }
            
            if (Auth::guard('sanghani_btc_user')->attempt($request->only('email', 'password'))) {
                $user = Auth::guard('sanghani_btc_user')->user();
    ...
    {
        try {
            $user = SanghaniBtcUser::where('email',$request->email)->first();
            if(!isset($user) && empty($user)){
                return $this->responseError('User not found', 401);
            }
    ...
    {
        try {
            $users = SanghaniBtcUser::with(['referredUsers.referredUser'])->find($request->user_id);
    ...
    public function startMining(MiningRequest $request)
    {
        try {
            $validatedData = $request->validated();
            $user = SanghaniBtcUser::where('id',$request->user_id)->update(['is_active' => $request->is_active,'mine' => $request->mine,'deactivation_time' => Carbon::now()->addHours(12)]);
    ...
    public function updateMiningTime(Request $request) {
        try {
            $record = SanghaniBtcUser::find($request->user_id);
            if($record->is_active) {
    ...
    public function updateProfile(Request $request)
    {
         ...
         try {
            $user = SanghaniBtcUser::where('id', $request->user_id)->first();
            if(empty($user)){
                return $this->responseError('User not found', 401);
            }
